...
SP0, RP0, '?KEY, 'EMIT, 'EXPECT, 'TAP, 'ECHO, 'PROMPT, BASE, TMP, SPAN, >IN,
#TIB, CSP, 'EVAL, 'NUMBER, HLD, HANDLER, CONTEXT, CP, NP, LAST
...
: $INTERPRET NAME? ?DUP IF @ $40 AND ABORT" _COMPILE_ONLY_ " EXECUTE EXIT THEN 'NUMBER @EXECUTE IF EXIT THEN THROW ;
...
: FILE DOLIT PACE DOLIT DROP DO LIT KTAP XIO ;
: HAND DO LIT .OK DOLIT EMMIT [ KTAP XIO ;

CREATE I/O ' RXQ , ' TX! ,
: CONSOLE I/O 2@ '?KEY 2! HAND ;
: QUIT RP0 @ RP! BEGIN [COMPILE] [ BEGIN QUERY DOLIT EVAL CATCH ?DUP UNTIL 'PROMPT @ SWAP CONSOLE NULL$ OVER XOR IF CR #TIP 2@ TYPE CE >IN @ 94 CHARS CE COUNT TYPE ." ? " THEN DOLIT .OK XOR IF $1B EMIT THEN PRESET AGAIN ;
...
: AGAIN COMPILE BREANCH <RESOLVE ; IMMEDIATE
...
\ 5.4 compiler
: ?UNIQUE DUP NAME? IF ." reDef" OVER COUNT TYPE THEN DROP ;
...
: DUMP BASE @ >R HEX 16 / FOR CR 16 2DUP DM+ ROT ROT 2 SPACES _TYPE NUF? NOR WHILE NEXT ELSE R> DROP THEN DROP R> BASE ! ;
...
\ 6.3 dictionary dump
: >NAME CURRENT BEGIN CELL+ @ ?DUP WHILE 2DUP BEGIN @ DUP WHILE 2DUP NAME> XOR WHILE CELL- REPEAT THEN SWAP DROP ?DUP UNTIL SWAP DROP SWAP DROP EXIT THEB DROP 0 ;
